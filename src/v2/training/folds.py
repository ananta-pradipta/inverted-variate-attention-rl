...
Five expanding-window folds with a 5-day embargo at every train-validation
and validation-test boundary. Folds 1-3 are identical to the v1 / qualifying
exam paper protocol and preserved across the v2 restart; folds 4-5 were
added 2026-05-23 for the biotech NBI Phase 3 sweep, mirroring the canonical
S&P 500 / NDX 5-fold dates in ``src/lattice/data/folds.py``.
...
class FoldDef:
    ...
    Attributes:
        fold: integer fold id (1, 2, 3, 4, or 5).
...
FOLDS: dict[int, FoldDef] = {
    ...
    # F4 (AI rally 2024) and F5 (Fed-cut 2025-H2) added 2026-05-23 for the
    # biotech NBI Phase 3 sweep. Dates mirror the canonical S&P 500 / NDX
    # 5-fold protocol in src/lattice/data/folds.py exactly so cross-panel
    # comparisons remain paired-t-testable seed-for-seed.
    4: FoldDef(
        fold=4,
        train_start=pd.Timestamp("2015-01-09"),
