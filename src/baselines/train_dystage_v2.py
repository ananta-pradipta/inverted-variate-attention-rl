...
def main() -> None:
    ...
    elif args.panel_kind in ("lattice_native", "nasdaq100", "djia30",
    ...
    if cfg.panel_kind == "lattice_native":
        cache_suffix = "_lattice_native"
    elif cfg.panel_kind == "nasdaq100":
        cache_suffix = "_nasdaq100"
    elif cfg.panel_kind == "djia30":
        cache_suffix = "_djia30"
    elif cfg.panel_kind == "biotech_nbi":
        cache_suffix = "_biotech_nbi"
    elif cfg.panel_kind == "biotech_nbi_enriched":
        # Same prices and active mask as biotech_nbi, so the graph
        # cache (built from log_returns + tradable_mask) is identical.
        # Reusing the suffix lets the enriched run hit the existing
        # cache from prior biotech_nbi runs.
        cache_suffix = "_biotech_nbi"
    else:
        cache_suffix = ""
