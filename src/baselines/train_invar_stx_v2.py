...
class InvarSTXV2Config(V2BaselineConfig):
    ...
    # F3 (fundamental L1 cross-stock attention; 2026-05-26 experiment):
    # Add an explicit cross-stock self-attention block at the END of the
    # per-stock encoder, AFTER day-memory fusion and BEFORE the score
    # head. Stocks attend TO EACH OTHER at the same timestep so the
    # score depends on relationships with other active stocks. The
    # canonical InvarSTXModel already has a cross-ticker iTransformer
    # Encoder in the backbone; F3 adds a SECOND, explicit cross-stock
    # block at the head boundary (closer to MASTER's structural fix).
    # Default False so the canonical path is byte-identical when this
    # flag is off.
    ...
    # "cs_mse" (default) preserves the canonical bankless+clpretrain
    # behaviour byte-identically. "listmle" adds a Plackett-Luce
    # rank-likelihood term to the per-day Stage-2 finetune objective
    # alongside the existing cross-sectional MSE; see
    # src/invar/training/loss.listmle_loss and
    # reports/path_to_baseline_beat_fundamental_2026-05-26.md.
    ...
    # finetune. All default to off / no-op: the diff_sharpe term is
    # only added when loss_config == "diff_sharpe", and even then the
    # weight defaults to 0.2 so the canonical Huber + listwise +
    # pairwise core stays load-bearing. K defaults match the SP500
    # wrapper (K=50); override per universe (NDX K=20, NBI K=25).
    # batch_days is the size of the day buffer over which the Sharpe
    # std is computed; default 16 days approximately one half month.
    ...
    # flags. All default to False / canonical so the Stage 2 finetune
    # path is byte-identical when cae_head_enabled is False.
    cae_head_enabled: bool = False
    cae_head_k_latent: int = 3
    cae_head_hidden_width: int = 64
    cae_head_dropout: float = 0.1
    ...
    cae_head_ridge_lambda: float = 1.0e-3
    cae_head_metrics_dir: str = ""
    # Robust-InVAR-RL Phase 1 (2026-05-26): group-DRO + top-bottom listwise
    # loss. All default to off; canonical path is byte-identical when
    # use_group_dro is False and lambda_top_bottom is 0.0.
    use_group_dro: bool = False
    eta_gdro: float = 0.05
    lambda_top_bottom: float = 0.10
    m_top_bottom: int = 50
    gdro_regime_cache_root: str = "cache/dr_rl/regime_probs"
    gdro_universe_id: str = "sp500"
    # B2 (2026-05-27): Stage-1 auxiliary next-day return regression head.
    # Adds a small nn.Linear(d_model, 1) head on top of the pretrain
    # backbone during Stage-1 ONLY (discarded before Stage-2 finetune;
    # only the backbone weights flow forward). Multi-task pretrain loss
    # becomes InfoNCE + pretrain_aux_regression_weight * cs_mse(score, y).
    # Both default to off / canonical so Stage-1 is byte-identical when
    # pretrain_aux_regression_head is False.
    ...
    # "kmeans" (default) preserves the canonical L2-nearest-neighbour
    # selector over the 14-d standardised episode-key fingerprint (this
    # is byte-identical to the canonical clpretrain). "hmm" swaps the
    # selector for a Gaussian HMM (hmmlearn preferred, sklearn GMM
    # fallback) posterior cosine-similarity selector; positives become
    # in-batch pairs whose posterior cosine similarity >= the threshold
    # below. See src/models/pretrain_improvements/hmm_regime.py for the
    # full posterior contract and disk cache layout.
    ...
    # "regime" (default) preserves the canonical / B1 day-level selector
    # path (byte-identical when pretrain_regime_method == "kmeans").
    # "sector" runs a per-day per-stock InfoNCE whose positives are
    # same-day same-sector peers and negatives are same-day different-
    # sector peers; sector ids come from cache/sector_labels/<id>.parquet
    # (see src/models/pretrain_improvements/sector_positives.py).
    ...
    # k-means-8 day-level selector byte-identically (composes with
    # pretrain_regime_method == "kmeans" and pretrain_positive_method ==
    # "regime"). "infonce_hmm" composes with the B1 HMM selector
    # (requires pretrain_regime_method == "hmm"). "infonce_sector"
    # composes with the C3 sector selector (requires pretrain_positive_
    # method == "sector"). "masked_feature" SKIPS the InfoNCE objective
    # entirely and runs a BERT/MAE-style masked-feature reconstruction
    # pretext via src/models/pretrain_improvements/masked_feature_
    # modeling.py; only the encoder is carried into Stage 2.
    pretrain_method: str = "infonce_kmeans"
    pretrain_mask_ratio: float = 0.15
    # A1 (2026-05-27): sequential multi-stage Stage-1 pretrain curriculum.
    # Default ["regime"] preserves the canonical single-stage path
    # byte-identically. ["regime", "sector"] runs Stage 1a with the
    # canonical k-means-8 day-level InfoNCE selector, then Stage 1b
    # continues training the SAME encoder weights with the C3 per-stock
    # same-sector InfoNCE selector. Each stage uses ``pretrain_epochs``
    # epochs. The final encoder is saved to the per-fold ckpt path.
    # A2 (2026-05-27): adds the "comovement" stage. ["regime",
    # "comovement"] = canonical regime InfoNCE for ``pretrain_epochs``
    # epochs (Stage 1a), then per-day per-stock SupCon InfoNCE over
    # same-co-movement-cluster positives for
    # ``pretrain_comovement_epochs`` epochs (Stage 1b). Co-movement
    # clusters are fit per fold on the TRAIN-segment returns matrix
    # (252-day rolling correlation, distance = 1 - rho, k-means with
    # K=``pretrain_comovement_n_clusters``); see
    # ``src/models/pretrain_improvements/comovement_clustering.py``.
    pretrain_stages: list[str] = field(default_factory=lambda: ["regime"])
    ...
    # A4 (2026-05-27): joint multi-objective Stage-1 pretrain. When
    # pretrain_positive_method == "joint" a single Stage-1 objective
    # aggregates THREE InfoNCE terms in every batch: the day-level
    # k-means-8 regime nearest-neighbour InfoNCE (canonical), the
    # per-stock same-GICS-sector SupCon InfoNCE (C3), and the per-stock
    # same-co-movement-cluster SupCon InfoNCE (A2). The total loss is
    # w_regime * L_regime + w_sector * L_sector + w_comove * L_comove.
    # Both cohort tensors (sector ids + co-movement cluster ids) are
    # loaded up front. The sector universe is read from
    # pretrain_sector_universe_id and the co-movement universe from
    # pretrain_comovement_universe_id (both default "sp500").
    ...
    nasdaq100_macro_duration_parquet: str = (
        "data/nasdaq100/macro.parquet"
    )
    djia30_macro_duration_parquet: str = (
        "data/djia30/macro.parquet"
    )
    biotech_nbi_macro_duration_parquet: str = (
        "data/biotech_nbi/macro.parquet"
    )
    ...
    nasdaq100_rolling_betas_parquet: str = (
        "data/processed/nasdaq100_rolling_betas.parquet"
    )
    djia30_rolling_betas_parquet: str = (
        "data/processed/djia30_rolling_betas.parquet"
    )
    biotech_nbi_rolling_betas_parquet: str = (
        "data/processed/biotech_nbi_rolling_betas.parquet"
    )
...
class CrossStockAttention(nn.Module):
    """F3: Explicit cross-stock self-attention over the active stocks.

    Multi-head self-attention applied to the per-stock representations
    of a single trading day. Stocks attend to each other, so the score
    for stock A can depend on relationships with every other active
    stock at the same timestep. This is the MASTER (Li et al. AAAI'24)
    structural lever: a stock-level attention layer at the head
    boundary that the canonical InVAR pre-head representation lacks
    (the canonical model has a cross-ticker iTransformer Encoder in
    the backbone, but the post-fusion representation only re-mixes
    cross-stock structure once; F3 adds a SECOND, late pass closer to
    the score head).

    The training loop in train_invar_stx_v2 / train_invar_clpretrain_v2
    passes ONLY the active stocks for each day (``x_win`` is sliced by
    ``active_t``), so the input to this block is already ``(N_active,
    d_model)``: no padding mask is required. Inactive stocks are never
    in the tensor and never produce a score.

    Architecture: pre-norm multi-head attention with a residual add,
    followed by a position-wise FFN with a residual add. Both norms are
    LayerNorm over d_model. The FFN width matches ``cfg.d_ff`` so the
    block is shape-compatible with the rest of the model.
    """

    def __init__(self, cfg: InvarSTXV2Config) -> None:
        super().__init__()
        d = cfg.d_model
        self.attn = nn.MultiheadAttention(
            embed_dim=d,
            num_heads=int(getattr(cfg, "cross_stock_heads", 4)),
            dropout=cfg.dropout,
            batch_first=True,
        )
        ...
        # Zero-init the attention out_proj and the last FFN Linear so
        # the block starts as an identity residual (no perturbation of
        # the canonical representation at init). The block only
        # contributes once gradients accumulate.
        with torch.no_grad():
            self.attn.out_proj.weight.zero_()
            self.attn.out_proj.bias.zero_()
            self.ffn[-2].weight.zero_()
            self.ffn[-2].bias.zero_()

    def forward(self, h: Tensor) -> Tensor:
        """Args:
            h: ``(N_active, d_model)`` per-stock representations.

        Returns:
            ``(N_active, d_model)`` with cross-stock structure added.
        """
        # Pre-norm self-attention. Batch=1, seq=N_active.
        x_in = self.norm1(h).unsqueeze(0)              # (1, N, d)
        attn_out, _ = self.attn(
            x_in, x_in, x_in, need_weights=False,
        )
        h = h + attn_out.squeeze(0)                    # (N, d)
        h = h + self.ffn(self.norm2(h))                # (N, d)
        return h
...
class InvarSTXModel(nn.Module):
    ...
    def __init__(
        ...
        # F3 (cross-stock attention; default off so the canonical
        # path is byte-identical when the flag is False).
        if getattr(cfg, "cross_stock_attn", False):
            self.cross_stock_attn_block = CrossStockAttention(cfg)
        else:
            self.cross_stock_attn_block = None
    ...
    def forward(
        ...
        # F3: explicit cross-stock attention at the head boundary.
        # Default behaviour (cross_stock_attn=False) is unchanged: the
        # block is None and the next line is a no-op.
        if self.cross_stock_attn_block is not None:
            h = self.cross_stock_attn_block(h)                 # (N, d)
...
def main() -> None:
    ...
    p.add_argument("--cross_stock_attn", action="store_true",
                   help="F3: enable explicit cross-stock self-attention "
                        "block at the head boundary. Off by default = "
                        "canonical InVAR; on adds the MASTER-style "
                        "stock-level attention layer.")
    p.add_argument("--cross_stock_heads", type=int, default=4,
                   help="Number of heads for the F3 cross-stock attn "
                        "block. Ignored if --cross_stock_attn is off.")
    ...
    cfg.cross_stock_attn = bool(args.cross_stock_attn)
    cfg.cross_stock_heads = int(args.cross_stock_heads)
    ...
    if cfg.panel_kind == "lattice_native":
        ...
        # Same NBI universe + dates as biotech_nbi; macro features are
        # date-keyed (not ticker-keyed) so the parquet is identical.
        macro_path = Path(cfg.biotech_nbi_macro_duration_parquet)
    else:
    ...
    if cfg.panel_kind == "lattice_native":
        ...
        # Same NBI universe as biotech_nbi; rolling betas are computed
        # from the prices parquet (shared across both panel kinds).
        betas_path = Path(cfg.biotech_nbi_rolling_betas_parquet)
    else:
