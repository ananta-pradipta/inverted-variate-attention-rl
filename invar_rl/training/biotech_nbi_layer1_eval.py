...
``(fold, seed)`` cell on the biotech NBI universe.
...
- panel_kind = ``biotech_nbi``; the v2 runner routes panel / mask /
  macro / betas paths to the biotech NBI parquets (see Phase 2 report
  and the ``panel_kind == "biotech_nbi"`` branches in
...
- lattice_bridge sanity gate for biotech_nbi is ``N >= 150`` (set in
  Phase 2, commit 7ded6ba). The biotech NBI panel ships 433 unique
  historical tickers (2014-2025), well above that bar.

Per cell, after training, this driver additionally persists:
- daily scores at outputs/biotech_nbi/layer1/scores/fold{F}_seed{S}.parquet
  (one row per test (day, ticker), float32 score)
- macro encodings at
  outputs/biotech_nbi/layer1/macro_enc/fold{F}_seed{S}.parquet
  (one row per test day, float32 macro encoder output of dim
  ``cfg.macro_out_dim``)
- per-cell metadata JSON at
  outputs/biotech_nbi/layer1/metrics/fold{F}_seed{S}.json
...
def _train_cell(
    ...
    """Train one (fold, seed) cell of canonical InVAR on biotech NBI.
    ...
    print(
        f"[biotech_nbi_layer1] fold={fold} seed={seed} device={device} "
    ...
    if not ckpt_path.exists():
        ...
        print(
            f"[biotech_nbi_layer1] stage 1 ckpt exists; reusing {ckpt_path}",
            flush=True,
        )
...
def _persist_scores_and_macro(
    ...
    if not full_path.exists():
        print(
            f"[biotech_nbi_layer1] WARN no full ckpt at {full_path}; "
    ...
    if score_rows:
        ...
        print(
            f"[biotech_nbi_layer1] wrote {scores_path}: "
    ...
    if macro_rows:
        ...
        print(
            f"[biotech_nbi_layer1] wrote {macro_path}: "
...
def main() -> int:
    ...
    if metrics_path.exists():
        print(
            f"[biotech_nbi_layer1] {metrics_path} already exists; "
            f"skipping cell",
            flush=True,
        )
        return 0
    ...
    print(
        f"[biotech_nbi_layer1] wrote {metrics_path}: "
