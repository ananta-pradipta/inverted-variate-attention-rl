"""NASDAQ-100 Phase 7 Layer 3 SAC Ablation 6: macro-conditional concentration.

Cross-universe replication of :mod:`invar_rl.training.sp500_layer3_sac_ablation6`
on the NASDAQ-100 panel. Tests whether a 2-D SAC action (exposure scalar +
concentration logit mapped to a daily top-K) beats the canonical 1-D
exposure scalar with fixed K=20 (NDX-100's per-side wrapper concentration
in :mod:`invar_rl.training.nasdaq100_layer3_sac`).

Mirrors the SP500 Ablation 6 driver byte-for-byte except:

1. ``panel_kind="nasdaq100"`` (vs ``"lattice_native"``).
2. Default Layer 1 ckpt root ``outputs/nasdaq100/layer1/_ckpt`` (vs
   ``invar_rl/results/stage1/_ckpt``).
3. ``K_MIN = 3``, ``K_MAX = 20`` (vs 5/50). NDX-100's universe is ~178
   tradable names (vs ~500 for SP500); the canonical wrapper uses K=20
   per side (1/5th of SP500), so Ablation 6 here uses K_t in [3, 20] to
   keep the action-space-to-universe ratio comparable.
4. Default ``--output-dir-root`` is ``outputs/nasdaq100/ablations/ablation6``.

The action layout, reward function, exposure-change band, regime-change
counter, observation pipeline, and SAC hyperparameters remain byte-for-byte
equivalent to the SP500 driver so the NDX vs SP500 delta is a pure
universe effect.

CLI::

    python -m invar_rl.training.nasdaq100_layer3_sac_ablation6 \\
        --fold F --seed S \\
        --total-timesteps 20000 \\
        --output-dir-root outputs/nasdaq100/ablations/ablation6
"""
...
# NASDAQ-100 Option A wrapper concentration: top-20 per side (canonical
# Phase 5 K=20), with action-controlled K_t in [3, 20]. Ratio to universe
# size matches the SP500 [5, 50] choice (universe ~178 vs ~500).
_K_MAX: int = 20
_K_MIN: int = 3
...
class VariableKExposureEnv(gym.Env):
    ...
    clipped to ``[K_MIN, K_MAX]`` (``3..20`` for NDX-100). The wrapper
    picks the top-K_t long and bottom-K_t short by InVAR score that day,
    equal-weight, then the exposure scalar multiplies the realised L/S
    return.
...
def run_one_cell(
    ...
    payload = {
        ...
        "model": (
            ...
            "Layer 2 SAC. Tests macro-conditional concentration vs fixed K=20 "
            "(NDX-100 canonical Phase 5 per-side concentration)."
        ),
        ...
        "k_min": int(_K_MIN),
        "k_max": int(_K_MAX),
        "action_space": "(exposure in [0, 1.5], k_logit in [-5, 5]) -> K_t in [3, 20]",
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "NASDAQ-100 Ablation 6: macro-conditional concentration via 2-D "
            "SAC action (exposure scalar + K_t in [3, 20])."
        )
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        default="outputs/nasdaq100/ablations/ablation6",
    )
    p.add_argument(
    ...
    p.add_argument(
        "--panel-kind", type=str, default="nasdaq100",
        choices=["nasdaq100", "lattice_native", "biotech"],
    )
    return p.parse_args()
