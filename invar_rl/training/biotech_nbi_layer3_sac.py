...
biotech NBI universe. The canonical SAC architecture, observation
...
- output paths under ``outputs/biotech_nbi/layer3/...``
...
top-K=200 cap (biotech NBI active count ~270 per day).
...
CLI::
    ...
    python -m invar_rl.training.biotech_nbi_layer3_sac \\
        --fold F --seed S --protocol {ls|lo} \\
        --total-timesteps 20000 \\
        --output-dir-root outputs/biotech_nbi/layer3
"""
...
def _build_layer2_cfg() -> Layer2Config:
    """Construct the biotech NBI Layer 2 config used by the QP precompute."""
...
def run_one_cell(
    ...
    print(
        f"[biotech_nbi_layer3] fold={fold} seed={seed} protocol={protocol} "
    ...
    print(
        f"[biotech_nbi_layer3] precompute tape sizes: "
    ...
    print(
        f"[biotech_nbi_layer3] training SAC for {total_timesteps:,} steps "
    ...
    print(
        f"[biotech_nbi_layer3] best val Sharpe={selector.best_sharpe:+.4f} "
    ...
    print(
        f"[biotech_nbi_layer3] wrote {out_path} "
    ...
    print(f"[biotech_nbi_layer3] wrote {summary_path}", flush=True)
    return payload
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir-root", type=str,
                   default="outputs/biotech_nbi/layer3")
...
def main() -> int:
    ...
    if out_path.exists() and summary_path.exists():
        print(
            f"[biotech_nbi_layer3] {out_path} + summary exist; skipping",
            flush=True,
        )
        return 0
    run_one_cell(
