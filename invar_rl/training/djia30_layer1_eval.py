"""DJIA-30 Phase 3 Layer 1 (canonical InVAR) training driver.
...
``(fold, seed)`` cell on the DJIA-30 universe.
...
- panel_kind = ``djia30``; the v2 runner routes panel / mask / macro /
  betas paths to the DJIA-30 parquets (see Phase 2 report and the
  ``panel_kind == "djia30"`` branches in
...
- daily scores at outputs/djia30/layer1/scores/fold{F}_seed{S}.parquet
  (one row per test (day, ticker), float32 score)
- macro encodings at outputs/djia30/layer1/macro_enc/fold{F}_seed{S}.parquet
  (one row per test day, float32 macro encoder output of dim
  ``cfg.macro_out_dim``)
- per-cell metadata JSON at outputs/djia30/layer1/metrics/fold{F}_seed{S}.json
...
Bug fix vs the NASDAQ-100 Phase 3 driver: the CUDA-to-numpy crash in
the score-persistence step is now fixed at the bridge level via
``invar_rl.data.lattice_bridge.LatticePanelBatch.day_inputs``, which
moves the per-day target slice through ``.detach().cpu()`` before
``.numpy()``. This driver therefore does not need a local workaround.
"""
...
def _train_cell(
    ...
    """Train one (fold, seed) cell of canonical InVAR on DJIA-30.
    ...
    print(
        f"[djia30_layer1] fold={fold} seed={seed} device={device} "
    ...
    if not ckpt_path.exists():
        ...
        print(
            f"[djia30_layer1] stage 1 ckpt exists; reusing {ckpt_path}",
            flush=True,
        )
...
def _persist_scores_and_macro(
    ...
    if not full_path.exists():
        print(
            f"[djia30_layer1] WARN no full ckpt at {full_path}; "
    ...
    if score_rows:
        ...
        print(
            f"[djia30_layer1] wrote {scores_path}: "
    ...
    if macro_rows:
        ...
        print(
            f"[djia30_layer1] wrote {macro_path}: "
...
def main() -> int:
    ...
    if metrics_path.exists():
        print(
            f"[djia30_layer1] {metrics_path} already exists; "
            f"skipping cell",
            flush=True,
        )
        return 0
    ...
    print(
        f"[djia30_layer1] wrote {metrics_path}: "
