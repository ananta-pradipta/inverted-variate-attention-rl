...
biotech NBI universe. For one ``(fold, seed)`` cell, this driver loads
...
CLI::
    ...
    python -m invar_rl.training.biotech_nbi_layer2_qp \
...
def _ensure_scores(
    ...
    full-state ckpt exists, regenerate via the biotech NBI Phase 3
    ...
    if scores_path.exists():
        if len(pd.read_parquet(scores_path, columns=["date"])) > 0:
            print(
                f"[biotech_nbi_layer2] scores parquet present at "
    ...
    print(
        f"[biotech_nbi_layer2] regenerating scores for fold={fold} "
        f"seed={seed} from {full_path}",
        flush=True,
    )
    from invar_rl.training.biotech_nbi_layer1_eval import (
...
def _run_protocol(
    ...
    print(
        f"[biotech_nbi_layer2] fold={fold} seed={seed} protocol={protocol}: "
    ...
    for di, day in enumerate(test_dates):
        ...
        try:
            ...
            print(f"[biotech_nbi_layer2] cov fail day={day_ts} err={exc}",
                  flush=True)
            qp_fail += 1
            continue
        ...
        if now - last_log > 30.0 or di == len(test_dates) - 1:
            print(
                f"[biotech_nbi_layer2] {protocol} fold={fold} seed={seed} "
    ...
    if rows:
        ...
        print(
            f"[biotech_nbi_layer2] wrote {out_path}: {len(wdf):,} non-zero "
            f"weights across {wdf['date'].nunique()} days",
            flush=True,
        )
    else:
        print(f"[biotech_nbi_layer2] WARN no non-zero weights for {out_path}",
              flush=True)
    return {
...
def main() -> int:
    ...
    for protocol in protocols:
        ...
        if out_path.exists():
            print(f"[biotech_nbi_layer2] {out_path} exists; reading summary",
                  flush=True)
    ...
    print(f"[biotech_nbi_layer2] wrote {summary_path}", flush=True)
    return 0
