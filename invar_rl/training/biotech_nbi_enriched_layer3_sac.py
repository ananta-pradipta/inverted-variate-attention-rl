"""Biotech NBI Phase 5 Layer 3 SAC controller training.

Direct mirror of :mod:`invar_rl.training.nasdaq100_layer3_sac` for the
biotech NBI ENRICHED universe. The canonical SAC architecture, observation
layout, reward function, and hyperparameters from
:mod:`invar_rl.layer3_control.{agent, env, observation, reward}` are
reused byte-for-byte. The only changes from the NDX-100 driver are:

- ``cfg.panel_kind = "biotech_nbi_enriched"``
- output paths under ``outputs/biotech_nbi_enriched/layer3/...``

Layer 2 QP hyperparameters are held byte-for-byte from the NDX-100
driver (Policy P1): gamma=5, per-name cap=0.05, cov_lookback=120,
top-K=200 cap (biotech NBI ENRICHED active count ~270 per day).

Per-checkpoint validation Sharpe selector and test-segment persistence
behaviour follow the NDX-100 driver exactly.

CLI::

    python -m invar_rl.training.biotech_nbi_enriched_layer3_sac \\
        --fold F --seed S --protocol {ls|lo} \\
        --total-timesteps 20000 \\
        --output-dir-root outputs/biotech_nbi_enriched/layer3
"""
...
# Biotech NBI Phase 4 Layer 2 QP hyperparameters (byte-for-byte the
# NDX-100 Phase 4 values; Policy P1). Active universe is ~270 per day
# so the top-K=200 cap actually bites here (cf. NDX-100 ~100 active).
_BIOTECH_NBI_ENRICHED_LAYER2: Dict[str, object] = dict(
...
def _build_layer2_cfg() -> Layer2Config:
    """Construct the biotech NBI ENRICHED Layer 2 config used by the QP precompute."""
    return Layer2Config(**_BIOTECH_NBI_ENRICHED_LAYER2)
...
class _ValSharpeSelector:
    """SB3 callback that picks the best-val-Sharpe checkpoint.

    Mirrors :class:`invar_rl.training.nasdaq100_layer3_sac._ValSharpeSelector`
    byte-for-byte; restore handles SAC only (Phase 5 is SAC-only).
    """
...
def run_one_cell(
    ...
    panel_end: str,
    panel_kind: str = "biotech_nbi_enriched",
    ...
    print(
        f"[biotech_nbi_enriched_layer3] fold={fold} seed={seed} protocol={protocol} "
    ...
    print(
        f"[biotech_nbi_enriched_layer3] precompute tape sizes: "
    ...
    # Build SAC with the canonical agent factory.
    ...
    print(
        f"[biotech_nbi_enriched_layer3] training SAC for {total_timesteps:,} steps "
    ...
    print(
        f"[biotech_nbi_enriched_layer3] best val Sharpe={selector.best_sharpe:+.4f} "
    ...
    print(
        f"[biotech_nbi_enriched_layer3] wrote {out_path} "
    ...
    payload = {
        ...
        "layer2_qp": _BIOTECH_NBI_ENRICHED_LAYER2,
        "weighting_mode": weighting_mode,
        "equal_topk_k": int(equal_topk_k),
    ...
    print(f"[biotech_nbi_enriched_layer3] wrote {summary_path}", flush=True)
    return payload


def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Biotech NBI Phase 5 Layer 3 SAC controller training."
    )
    ...
    p.add_argument("--output-dir-root", type=str,
                   default="outputs/biotech_nbi_enriched/layer3")
    p.add_argument("--layer1-ckpt-root", type=str,
                   default="outputs/biotech_nbi_enriched/layer1/_ckpt")
    ...
    p.add_argument(
        ...
        help=(
            "Layer 2 weighting mode: 'qp' (default, MV-QP with "
            "per_name_bound=0.05) or 'equal_topk' (equal-weight long "
            "top-K / short bottom-K, K per side via --equal-topk-k). "
            "equal_topk gives apples-to-apples K matching with the "
            "Phase 5.5 baseline rankers."
        ),
    )
    p.add_argument(
        ...
        help=(
            ...
            "QP). Default 50 mirrors the SP500 Phase 6 equal_l2 K; "
            "pass 25 for fair comparison against the K=25 NBI baselines."
        ),
    )
    return p.parse_args()
...
def main() -> int:
    ...
    if out_path.exists() and summary_path.exists():
        print(
            f"[biotech_nbi_enriched_layer3] {out_path} + summary exist; skipping",
            flush=True,
        )
        return 0
    run_one_cell(
