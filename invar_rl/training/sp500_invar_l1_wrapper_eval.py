"""S&P 500 InVAR Layer 1 + top-25 L/S wrapper eval (Panel A row).

Direct mirror of :mod:`invar_rl.training.nasdaq100_invar_l1_wrapper_eval`
for the universal S&P 500 ``lattice_native`` panel. For each
``(fold, seed)`` cell on the SP500 universe, this script:

  1. Builds the universal ``lattice_bridge`` for the fold (canonical
     panel routing, tradable mask, 1-day log-return panel).
  2. Loads the canonical InVAR Layer-1 full state_dict from
     ``invar_rl/results/stage1/_ckpt/fold{F}_seed{S}_full.pt``
     (written by ``invar_rl/training/stage1_rank.py`` when
  ...
  4. Applies the SAME top-25 long / bottom-25 short wrapper used for
     MASTER / FactorVAE / etc. on NDX-100 (gross 2.0, net 0.0, daily
     rebalance, equal weight, dollar-neutral). The shared
     :func:`invar_rl.training.nasdaq100_baseline_eval._topk_long_short_portfolio`
     is reused to guarantee byte-for-byte methodological symmetry with
     the NDX-100 row.
  5. Writes
     ``outputs/sp500/baselines/invar_l1/fold{F}_seed{S}.json``
     with the same JSON schema as the NDX-100 InVAR L1 wrapper row.

The pooled per-cell mean Sharpe across the 25 cells is the SP500
"InVAR L1 + top-25 L/S wrapper" row in Panel A of the InVAR-RL paper.

Usage (per cell)::

    PYTHONPATH=$PWD python3 -m invar_rl.training.sp500_invar_l1_wrapper_eval \\
        --fold 1 --seed 42
...
_TOP_K_LS_NATIVE: int = _TOP_K_LS  # 25, symmetric with NDX Panel A row.


def _build_bridge(fold: int, panel_end: str, two_regime_val: bool,
                  device: torch.device):
    """Build the SP500 (lattice_native) bridge for one fold (seed-agnostic)."""
...
def _load_trained_invar(
    ...
    if not full_path.exists():
        raise FileNotFoundError(
            ...
            f"Run stage1_rank.py with INVAR_SAVE_FULL_STATE=1 "
            f"to produce it."
        )
    ckpt = torch.load(full_path, map_location=device)
    # F3 (cross-stock attention) flags; legacy ckpts default to off.
    cfg.cross_stock_attn = bool(ckpt.get("cross_stock_attn", False))
    cfg.cross_stock_heads = int(ckpt.get("cross_stock_heads", 4))
...
def _build_score_matrix(model, bridge, device: torch.device) -> np.ndarray:
    ...
    npz ``y_hat`` schema consumed by the Phase A baseline harness.
    """
    ...
    payload = {
        ...
        "config": {
            ...
            "note": (
                ...
                "Isolates ranker quality on SP500."
            ),
        },
    }
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "SP500 Panel A row: InVAR Layer 1 + top-25 L/S wrapper "
    ...
    p.add_argument(
        ...
        default="outputs/sp500/baselines",
