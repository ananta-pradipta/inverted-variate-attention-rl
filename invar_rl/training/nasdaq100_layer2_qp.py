"""NASDAQ-100 Phase 4 Layer 2 mean-variance QP driver.

For one ``(fold, seed)`` cell on the NASDAQ-100 universe, this driver
loads (or regenerates) the Layer 1 scores parquet, estimates a daily
...
CLI::
    ...
    python -m invar_rl.training.nasdaq100_layer2_qp \
...
def _ensure_scores(
    ...
    Phase 3 NASDAQ-100 lost its scores parquets to the lattice_bridge
    CUDA-to-numpy bug fixed in commit d47860d. If the parquet is
    missing but the full-state ckpt exists, regenerate via the Phase 3
    ...
    if scores_path.exists():
        if len(pd.read_parquet(scores_path, columns=["date"])) > 0:
            print(
                f"[layer2] scores parquet present at {scores_path}; "
                f"skipping regen",
                flush=True,
            )
            return scores_path
    ...
    print(
        f"[layer2] regenerating scores for fold={fold} seed={seed} "
        f"from {full_path}",
        flush=True,
    )
    from invar_rl.training.nasdaq100_layer1_eval import (
...
def _run_protocol(
    ...
    print(
        f"[layer2] fold={fold} seed={seed} protocol={protocol}: "
    ...
    for di, day in enumerate(test_dates):
        ...
        try:
            ...
            print(f"[layer2] cov fail day={day_ts} err={exc}", flush=True)
            qp_fail += 1
            continue
        ...
        if now - last_log > 30.0 or di == len(test_dates) - 1:
            print(
                f"[layer2] {protocol} fold={fold} seed={seed} "
    ...
    if rows:
        ...
        print(
            f"[layer2] wrote {out_path}: {len(wdf):,} non-zero weights "
            f"across {wdf['date'].nunique()} days",
            flush=True,
        )
    else:
        print(f"[layer2] WARN no non-zero weights for {out_path}",
              flush=True)
    return {
...
def main() -> int:
    ...
    for protocol in protocols:
        ...
        if out_path.exists():
            print(f"[layer2] {out_path} exists; reading summary stats",
                  flush=True)
    ...
    print(f"[layer2] wrote {summary_path}", flush=True)
    return 0
