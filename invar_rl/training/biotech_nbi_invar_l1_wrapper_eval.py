...
NBI universe. For each ``(fold, seed)`` cell:
  ...
  1. Builds the biotech NBI lattice_bridge for the fold.
  2. Loads the canonical InVAR Layer-1 full state_dict from
     ``outputs/biotech_nbi/layer1/_ckpt/fold{F}_seed{S}_full.pt``.
  ...
  5. Writes
     ``outputs/biotech_nbi/baselines/invar_l1/fold{F}_seed{S}.json``.

The wrapper isolates ranker quality (no QP, no SAC).

Usage::

    PYTHONPATH=$PWD python3 -m invar_rl.training.biotech_nbi_invar_l1_wrapper_eval \\
        --fold 1 --seed 42
...
def _build_bridge(fold: int, panel_end: str, two_regime_val: bool,
                  device: torch.device):
    """Build the biotech NBI lattice bridge for one fold (seed-agnostic)."""
...
def _load_trained_invar(
    ...
    if not full_path.exists():
        raise FileNotFoundError(
            ...
            f"Run biotech_nbi_layer1_eval.py with "
