"""NASDAQ-100 Layer 1 with B1 (HMM regime pretrain) selector.

Per-universe Stage 1 entrypoint for the B1 (2026-05-27) cross-universe
transfer test. Mirrors the SP500 B1 path that the canonical SP500 sbatch
...
``InvarSTXV2Config`` for NASDAQ-100 with the B1 HMM cfg fields set,
then calls the canonical pretrain + finetune from
...
Per-fold encoder ckpt lives at ``outputs/nasdaq100/layer1_b1_hmm/_ckpt/
fold{F}_encoder.pt`` and is shared across the 5 finetune seeds (42..46).
The HMM posterior cache lives at
``cache/pretrain_improvements/hmm_regime/nasdaq100/fold{F}/posteriors.parquet``.

Usage::

    python -m invar_rl.training.nasdaq100_layer1_b1_hmm \
        --fold 1 --seed 42 \
        --output-dir outputs/nasdaq100/layer1_b1_hmm \
        --pretrain-epochs 10 --finetune-epochs 10 \
        --pretrain-hmm-n-states 4 \
        --pretrain-hmm-positive-threshold 0.7 \
        --pretrain-hmm-universe-id nasdaq100
...
def _build_cfg(
    ...
    """Build the canonical NASDAQ-100 InVAR cfg with B1 HMM flags set."""
...
def main() -> int:
    ...
    p.add_argument(
        "--output-dir", type=str,
        default="outputs/nasdaq100/layer1_b1_hmm",
    )
    ...
    p.add_argument(
        "--pretrain-hmm-universe-id", type=str, default="nasdaq100",
        help=(
    ...
    print(
        f"[nasdaq100_layer1_b1_hmm] fold={cfg.fold} seed={cfg.seed} "
        f"panel={cfg.panel_kind} hmm_n_states={cfg.pretrain_hmm_n_states} "
    ...
    if not args.skip_pretrain:
        if ckpt_path.exists():
            print(
                f"[nasdaq100_layer1_b1_hmm] stage 1 ckpt exists at "
