...
For each ``(baseline, fold, seed)`` cell:
  ...
  2. Builds the biotech NBI ``lattice_bridge`` for the (fold).
  ...
  4. Writes ``outputs/biotech_nbi/baselines/{baseline}/fold{F}_seed{S}.json``.

Usage (per cell)::

    python -m invar_rl.training.biotech_nbi_baseline_eval \\
...
Usage (rollup only)::
    ...
    python -m invar_rl.training.biotech_nbi_baseline_eval --rollup
"""
...
_NATIVE_K: Dict[str, int] = {
    ...
    # invar_rl/training/biotech_nbi_invar_l1_wrapper_eval.py.
    "invar_l1": 25,
}
...
def _build_bridge_for_fold(
    ...
    """Build the biotech NBI lattice bridge for one fold.
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--npz-root", type=str,
        default="results/baselines_biotech_nbi_two_regime_val",
        help="Root directory of saved per-(fold, seed) prediction npz.",
    )
    p.add_argument(
        "--output-dir-root", type=str,
        default="outputs/biotech_nbi/baselines",
