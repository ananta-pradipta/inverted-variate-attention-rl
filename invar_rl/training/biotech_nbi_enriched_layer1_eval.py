"""Biotech NBI Phase 3 Layer 1 (canonical InVAR) training driver.
...
``(fold, seed)`` cell on the biotech NBI ENRICHED universe.

Constraints (per the Phase 3 spec, Policy P1 byte-for-byte S&P 500):
...
- panel_kind = ``biotech_nbi``; the v2 runner routes panel / mask /
  macro / betas paths to the biotech NBI ENRICHED parquets (see Phase 2 report
  and the ``panel_kind == "biotech_nbi_enriched"`` branches in
  ...
  feeds).
- lattice_bridge sanity gate for biotech_nbi is ``N >= 150`` (set in
  Phase 2, commit 7ded6ba). The biotech NBI ENRICHED panel ships 433 unique
  historical tickers (2014-2025), well above that bar.

Per cell, after training, this driver additionally persists:
- daily scores at outputs/biotech_nbi_enriched/layer1/scores/fold{F}_seed{S}.parquet
  (one row per test (day, ticker), float32 score)
- macro encodings at
  outputs/biotech_nbi_enriched/layer1/macro_enc/fold{F}_seed{S}.parquet
  (one row per test day, float32 macro encoder output of dim
  ``cfg.macro_out_dim``)
- per-cell metadata JSON at
  outputs/biotech_nbi_enriched/layer1/metrics/fold{F}_seed{S}.json
...
Carries the DJIA-30 / NASDAQ-100 lattice_bridge CUDA-to-numpy fix
forward (no local workaround needed; see
``invar_rl.data.lattice_bridge.LatticePanelBatch.day_inputs``).
"""
...
def _train_cell(
    ...
    """Train one (fold, seed) cell of canonical InVAR on biotech NBI ENRICHED.
    ...
    runs it once per cell and short-circuits subsequent calls when the
    ckpt already exists (the canonical trainer's stage 2 reads the
    ckpt back regardless of whether we just trained it or reused it).
    ...
    print(
        f"[biotech_nbi_enriched_layer1] fold={fold} seed={seed} device={device} "
    ...
    if not ckpt_path.exists():
        ...
        print(
            f"[biotech_nbi_enriched_layer1] stage 1 ckpt exists; reusing {ckpt_path}",
            flush=True,
        )
...
def _persist_scores_and_macro(
    ...
    Note: relies on the lattice_bridge ``.cpu()`` fix for tensors that
    are placed on CUDA via ``build_lattice_bridge(cfg, device=device)``.
    The day_inputs slice now moves targets through CPU before numpy
    conversion, so the score persistence step does not crash on GPU.
    """
    import pandas as pd
    ...
    if not full_path.exists():
        print(
            f"[biotech_nbi_enriched_layer1] WARN no full ckpt at {full_path}; "
    ...
    if score_rows:
        ...
        print(
            f"[biotech_nbi_enriched_layer1] wrote {scores_path}: "
    ...
    if macro_rows:
        ...
        print(
            f"[biotech_nbi_enriched_layer1] wrote {macro_path}: "
...
def main() -> int:
    ...
    p.add_argument("--universe", type=str, default="biotech_nbi_enriched",
                   choices=["biotech_nbi_enriched"])
    ...
    if metrics_path.exists():
        print(
            f"[biotech_nbi_enriched_layer1] {metrics_path} already exists; "
            f"skipping cell",
            flush=True,
        )
        return 0
    ...
    print(
        f"[biotech_nbi_enriched_layer1] wrote {metrics_path}: "
