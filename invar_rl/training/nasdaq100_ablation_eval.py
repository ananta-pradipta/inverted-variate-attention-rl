"""NASDAQ-100 Phase 6 four-ablation evaluator.

Single parametrised driver for the Phase 6 four-ablation replication on
the NASDAQ-100 universe. One run trains (or rolls) ONE (ablation,
method, fold, seed, protocol) cell and persists the same parquet +
summary JSON pair as :mod:`invar_rl.training.nasdaq100_layer3_sac`, so
the Phase 6 rollup can be cross-checked against the Phase 5 canonical
SAC numbers byte-for-byte.

This driver mirrors the SP500 Phase 6 ablation
(``invar_rl.training.stage3_rl_ablation``) byte-for-byte where possible
(Policy P1 reuse): the precompute tape, observation-strip wrapper, RL
agent factory, env, reward function, and val-Sharpe selector are all
imported from the same modules; only the universe-keyed branches
(``panel_kind="nasdaq100"``), the NDX-100 Layer 2 QP hyperparameters
(reused from :mod:`invar_rl.training.nasdaq100_layer3_sac`), and the
NDX-100-appropriate equal-weight top-K (K=20 per side; SP500 uses
K=50) are NDX-specific.

Ablation conditions:

- ``canonical``: identity (no ablation). Reproduces Phase 5 SAC for
  ``--method sac`` and adds recurrent_ppo, feedforward_ppo,
  constant_full on the same NDX-100 stack.
- ``random_l1``: Layer 1 InVAR scores replaced with i.i.d. N(0, 1)
  noise (seeded by ``--seed``).
- ``equal_l2``: Layer 2 cvxpy QP replaced with equal-weight top-K /
  bottom-K (K=20 per side for NDX-100; K=50 for SP500). Layer 1 stays
  canonical.
- ``stripped_l3``: Layer 1 + Layer 2 canonical, but the Layer-1 /
  Layer-2 fields of the RL observation (score_dispersion, pred_vol,
  eff_positions, full macro encoding) are zeroed via
  :class:`StrippedObservationWrapper`. ``constant_full`` is unaffected
  by this ablation (it does not consume the observation) and is
  skipped.

Methods:

- ``recurrent_ppo``, ``feedforward_ppo``, ``sac`` (RL controllers
  trained via SB3 + sb3-contrib).
- ``constant_full``: holds the L1 + L2 portfolio with exposure 1.0
  (no Layer 3 intervention). No training.

Output schema matches Phase 5: per-day exposures + realised portfolio
returns parquet at
``outputs/nasdaq100/phase6_ablation/{ablation}/{method}/{protocol}/foldF_seedS.parquet``
and a per-cell summary JSON under the same path with a ``summary``
subdir. The summary JSON also carries
``tape_constant_full_mean_return`` / ``tape_constant_full_volatility``
so the Layer 1 + Layer 2 base-book Sharpe is recoverable from any RL
cell without re-running constant_full separately.

CLI::

    python -m invar_rl.training.nasdaq100_ablation_eval \\
...
(unaffected) so the actual grid is 3 + 4 + 4 + 4 = 15 method/ablation
pairs x 25 cells x 2 protocols = 750 cells. The Phase 5 SAC canonical
L/S + L/O has already been completed (50 cells), so 700 new cells are
required.
"""
...
# NASDAQ-100 Phase 4 Layer 2 QP hyperparameters; identical to the
# Phase 5 SAC driver. Authoritative copy lives in nasdaq100_layer3_sac.py.
...
_TRADING_DAYS = 252
# Per-side K for the equal_l2 ablation on NDX-100. The active universe
# is roughly 100 names vs the SP500 ablation's ~500, so K=20 keeps the
# fraction-of-universe identical (20/100 = 50/250). For long-only,
# this is the total top-K count.
_EQUAL_L2_K_NDX = 20
...
def _rollout_rl(
    ...
    """Deterministic RL rollout; return realised returns + exposures."""
...
def _rollout_constant_full(
    ...
    """Rollout the constant-full-exposure baseline through ExposureEnv."""
...
class _ValSharpeSelector:
    ...
    Mirrors :class:`invar_rl.training.nasdaq100_layer3_sac._ValSharpeSelector`
    but supports SAC and PPO (recurrent + feedforward) by branching on
    the agent class during restore.
    """
    ...
    def restore_best(self, agent, method: str) -> bool:
        """Load the best-val checkpoint back into ``agent``."""
...
def _persist_test_outputs(
    ...
    """Write per-test-day exposures + realised returns parquet."""
...
def run_one_cell(
    ...
    print(
        f"[nasdaq100_ablation] ablation={ablation} method={method} "
    ...
    print(
        f"[nasdaq100_ablation] tape sizes: "
    ...
    if method in _RL_METHODS:
        ...
        print(
            f"[nasdaq100_ablation] training {method} for "
        ...
        print(
            f"[nasdaq100_ablation] best val Sharpe="
    ...
    else:  # constant_full (no training, no observation needed)
    ...
    print(
        f"[nasdaq100_ablation] wrote {out_path} "
        f"(test pooled Sharpe={test_stats['sharpe']:+.4f}, "
        f"n_days={test_stats['n_test_days']})",
        flush=True,
    )

    # Base-book (Layer 1 + Layer 2 with no Layer 3 intervention) tape
    # statistics: identical for any RL method at the same (ablation,
    # fold, seed, protocol), recorded so the per-cell tape baseline is
    # auditable from every JSON.
    ...
    payload: Dict[str, object] = {
        ...
        "equal_topk_k": int(_EQUAL_L2_K_NDX),
    ...
    print(f"[nasdaq100_ablation] wrote {summary_path}", flush=True)
    return payload
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default="outputs/nasdaq100/phase6_ablation",
    )
    p.add_argument(
...
def main() -> int:
    args = _parse_args()
    # stripped_l3 + constant_full is an undefined combination (constant
    # exposure does not consume the observation). Skip rather than fail.
    if args.ablation == "stripped_l3" and args.method == "constant_full":
        print(
            "[nasdaq100_ablation] skipping stripped_l3 + constant_full "
    ...
    if out_path.exists() and summary_path.exists():
        print(
            f"[nasdaq100_ablation] skip {out_path} + summary "
