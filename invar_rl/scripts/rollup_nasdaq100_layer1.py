"""Rollup the NASDAQ-100 Phase 3 Layer 1 sweep results.

Reads every ``outputs/nasdaq100/layer1/metrics/fold{F}_seed{S}.json``
written by ``invar_rl/training/nasdaq100_layer1_eval.py``, computes the
per-fold pooled rank IC (mean and std across seeds), the all-25-cell
pooled rank IC (mean and std), and the canonical-vs-S&P-500 comparison.
Patches the numbers into ``reports/nasdaq100/phase_3_layer1_ic.md``
(replacing the TBD rows) and prints a CLI summary.

Usage::

    PYTHONPATH=. python3 -m invar_rl.scripts.rollup_nasdaq100_layer1
"""
...
METRICS_DIR_DEFAULT = Path("outputs/nasdaq100/layer1/metrics")
REPORT_PATH_DEFAULT = Path("reports/nasdaq100/phase_3_layer1_ic.md")

SP500_POOLED_RANK_IC = 0.0284
...
def _patch_report(
    ...
    cmp_block = (
        "## Comparison vs S&P 500 (lattice_native canonical InVAR)\n\n"
        "| Universe | Pooled rank IC | Std |\n"
        "|----------|----------------|-----|\n"
        f"| sp500    | +0.0284 (canonical, locked 2026-05-19) | "
        f"{SP500_POOLED_STD:.3f} |\n"
        f"| nasdaq100 | {pooled['mean']:+.4f} | {pooled['std']:.4f} |\n"
        f"| ratio (nasdaq100 / sp500) | {ratio} | n/a |\n"
    )
    text = re.sub(
