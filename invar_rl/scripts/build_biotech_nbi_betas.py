"""Build the biotech NBI per-(day, ticker) rolling macro-beta panel.

Mirrors ``src.v2.data.rolling_macro_betas.build_rolling_betas`` byte-for-byte
on the biotech NBI universe. Writes to
``data/processed/biotech_nbi_rolling_betas.parquet`` with the SAME
...
``data/biotech_nbi/macro.parquet`` which carries 32 of the same 32
universe-shared columns as the S&P 500 macro parquet (byte-identical
on the overlap; see ``reports/biotech_nbi/phase_2_report.md``).

The spec for biotech_nbi mentions a "rolling beta vs SPY over a 252-day
window" as a one-liner. The schema chosen here is the byte-for-byte
superset that the existing InVAR-RL lattice_bridge already expects (the
canonical 60-day / 120-day rolling ridge OLS on 5 macro factors). The
SPY-beta-252d concept is subsumed by the rolling_spy_beta_60d column;
extending it to a 252d window would require a separate column for the
bridge to consume, which the canonical InVAR schema does not yet
support.
"""
...
PRICES_PATH = REPO_ROOT / "data" / "biotech_nbi" / "prices.parquet"
MACRO_PATH = REPO_ROOT / "data" / "biotech_nbi" / "macro.parquet"
OUT_PATH = REPO_ROOT / "data" / "processed" / "biotech_nbi_rolling_betas.parquet"
...
def main() -> int:
    ...
    print(
        f"[biotech_nbi_betas] prices: {len(raw):,} rows; "
    ...
    print(
        f"[biotech_nbi_betas] panel dates: {len(panel_dates)}; "
    ...
    print(
        f"[biotech_nbi_betas] wrote {OUT_PATH}: shape={long.shape}, "
