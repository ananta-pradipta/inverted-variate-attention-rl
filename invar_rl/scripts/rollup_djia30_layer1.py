"""Rollup the DJIA-30 Phase 3 Layer 1 sweep results.

Reads every ``outputs/djia30/layer1/metrics/fold{F}_seed{S}.json``
written by ``invar_rl/training/djia30_layer1_eval.py``, computes the
per-fold pooled rank IC (mean and std across seeds), the all-25-cell
pooled rank IC (mean and std), and the canonical-vs-S&P-500 (and
parked NASDAQ-100) comparison. Patches the numbers into
``reports/djia30/phase_3_layer1_ic.md`` (replacing the TBD rows) and
prints a CLI summary.

Usage::

    PYTHONPATH=. python3 -m invar_rl.scripts.rollup_djia30_layer1
"""
...
METRICS_DIR_DEFAULT = Path("outputs/djia30/layer1/metrics")
REPORT_PATH_DEFAULT = Path("reports/djia30/phase_3_layer1_ic.md")

# Canonical S&P 500 InVAR pooled rank IC (locked 2026-05-19) and
# parked NASDAQ-100 (Phase 3 reference, see
# project_memory/project_next_steps_2026-05-16.md headline).
...
def _patch_report(
    ...
    cmp_block = (
        "## Comparison vs S&P 500 (canonical) and NASDAQ-100 (parked)\n\n"
        ...
        f"| djia30   | {pooled['mean']:+.4f} | {pooled['std']:.4f} |\n"
        f"| ratio (djia30 / sp500) | {ratio_sp500} | n/a |\n"
        f"| ratio (djia30 / nasdaq100) | {ratio_nasdaq} | n/a |\n"
    )
    text = re.sub(
