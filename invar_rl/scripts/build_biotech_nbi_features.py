"""Build biotech NBI per-ticker per-day technical features for Phase 2.
...
columns (per the Phase 2 spec):
    ...
    adv_dollar_20d          (20-day rolling mean dollar volume; auxiliary,
                             also consumed by the active-mask builder)

NO sector column is computed: the biotech NBI universe is
sector-degenerate (every constituent is biotech / healthcare) so any
per-stock sector encoding would be a constant feature with zero
cross-sectional variance, which the cross-sectional standardiser
would collapse to zeros anyway. The 26-col canonical panel built by
``build_biotech_nbi_panel.py`` zero-fills its slot in the 16
non-PRICE_VOL columns.

Output:
    data/biotech_nbi/features/year={YYYY}/part.parquet
...
def main() -> int:
    ...
    if not PRICES_PATH.exists():
        print(f"ERROR: missing {PRICES_PATH}; run "
              f"build_biotech_nbi_prices.py first.", flush=True)
        return 1
