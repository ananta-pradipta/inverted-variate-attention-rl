"""Build the biotech NBI daily active mask for Phase 2.

A (ticker, date) row is ACTIVE iff ALL of:
  - in_index_flag is True per data/biotech_nbi/membership.parquet, AND
  - the ticker has a non-NaN close on that date in
    data/biotech_nbi/prices.parquet, AND
  ...
  - the ticker's 20-day rolling average dollar volume on that date is
    >= 1e6 USD (liquidity filter; biotech small-caps frequently fail
    this gate so it is more binding here than on NDX-100 / DJIA-30).

Output: data/biotech_nbi/active_mask.parquet
    date     datetime64[ns]
    ticker   string
    active   bool
"""
