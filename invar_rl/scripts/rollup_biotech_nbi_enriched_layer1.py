"""Rollup the biotech NBI ENRICHED Phase 3 Layer 1 sweep results.

Reads every ``outputs/biotech_nbi_enriched/layer1/metrics/fold{F}_seed{S}.json``
written by ``invar_rl/training/biotech_nbi_enriched_layer1_eval.py``, computes
...
``reports/biotech_nbi/enriched_phase_3_layer1_ic.md`` (replacing the TBD rows)
and prints a CLI summary.

Usage::

    PYTHONPATH=. python3 -m invar_rl.scripts.rollup_biotech_nbi_enriched_layer1
"""
...
METRICS_DIR_DEFAULT = Path("outputs/biotech_nbi_enriched/layer1/metrics")
REPORT_PATH_DEFAULT = Path("reports/biotech_nbi/enriched_phase_3_layer1_ic.md")
...
def _patch_report(
    ...
    cmp_block = (
        ...
        f"| ratio (biotech_nbi_enriched / sp500) | {ratio_sp500} | n/a |\n"
        f"| ratio (biotech_nbi_enriched / nasdaq100) | {ratio_nasdaq} | n/a |\n"
        f"| ratio (biotech_nbi_enriched / djia30) | {ratio_djia} | n/a |\n"
    )
    text = re.sub(
