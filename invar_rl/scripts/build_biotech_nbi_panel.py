"""Build the biotech NBI 26-col canonical panel for InVAR Layer 1.

Mirrors the lattice_native 26-col schema (see
``src.v2.data.lattice_native_panel.FEATURE_COLS``) so the canonical
InVAR model code can ingest the biotech NBI panel with no architecture
changes (same ``feature_dim = 26``).

Computes the 10 PRICE_VOL columns from
``data/biotech_nbi/prices.parquet`` verbatim from
``src.lattice.data.build_panel._compute_price_vol`` math. Zero-fills
the remaining 16 columns (4 distress, 4 intangible, 3 other
fundamental, 3 catalyst, 2 flag) because the biotech NBI universe
...
No sector column is added: the biotech NBI universe is
sector-degenerate (every constituent is a biotech / healthcare name),
which is why a per-stock sector encoding would have zero
cross-sectional variance and offer no ranking signal.

Output:
    data/biotech_nbi/panel_features.parquet
...
def main() -> int:
    ...
    print(
        f"[biotech_nbi_panel] prices: {len(prices):,} rows; "
    ...
    print(
        f"[biotech_nbi_panel] dropped {pre_drop - len(panel):,} rows with "
    ...
    print(
        f"[biotech_nbi_panel] wrote {OUT_PATH}: "
