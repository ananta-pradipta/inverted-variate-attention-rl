"""Build the DJIA-30 daily OHLCV price panel for Phase 2.

Reads :mod:`data/djia30/membership.parquet` for the historical ticker
universe (41 unique symbols across 2014-2025) and downloads daily bars
from yfinance with ``auto_adjust=True`` so the close column is already
split- and dividend-adjusted.

Output: ``data/djia30/prices.parquet`` (long-format).
...
The UTX -> RTX rename (2020-04-03; United Technologies + Raytheon =>
Raytheon Technologies) is resolved via ``aliases.parquet``. UTX was
out of the DJIA before the rename so this is metadata-only; the
modern symbol RTX carries pre-rename history transparently and is
split by the rename date at persist time.

This script is idempotent; it writes to a temp parquet then
atomically renames. Use ``--no-cache`` to force a full re-download.

DJIA-30 names are all megacap blue chips; yfinance is expected to
serve all 41 unique tickers without failure.
"""
