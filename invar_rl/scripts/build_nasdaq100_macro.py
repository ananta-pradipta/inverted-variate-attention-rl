"""Build the NASDAQ-100 daily macro-state vector for Phase 2.
...
A single universe-specific column ``cs_avg_pairwise_corr_60d_nasdaq100``
is appended: the cross-sectional average pairwise correlation over a
trailing 60-day window computed from the NASDAQ-100 active-subset log
returns.
...
information available by day t-5. This matches the spec's "5-day
release-lag for macro features" guidance.

Inputs:
    data/raw/macro_fred_full.csv          (FRED DGS3MO, DGS2, DGS10, BAA10Y;
                                           auto-extended if stale)
    data/raw/sp500/sector_etfs.parquet    (SPY, QQQ, XLK, XLF, ...; auto-
                                           extended if stale)
    data/processed/risk_features_sp500.parquet  (vix, vxn, vvix,
                                                 vix_term_slope, ...)
    data/nasdaq100/prices.parquet         (Phase 2 prices)
    data/nasdaq100/active_mask.parquet    (Phase 2 active mask; rebuilt
                                           inline if missing)
...
REQUIRED_START = pd.Timestamp("2014-09-01")  # 60-day warmup buffer
...
def _ensure_risk_features() -> None:
    ...
    Falls back to yfinance ^VIX if the FRED cache lacks VIXCLS. Other
    columns (vxn, vvix, vix_term_slope, xbi_rv_*) are left NaN in the
    extension window; the build_universal_macro_duration_features step
    will ffill within 5 days and zero-fill thereafter on standardize.
    """
    if not RISK_SP500_PARQUET.exists():
        # Build a minimal risk_features file by pulling VIX from Yahoo.
        import yfinance as yf
...
def _build_universe_corr(
    ...
    For each business day t, take the active NASDAQ-100 subset (per
    ...
    # Active mask gate (in-index AND valid close).
    if active_mask_path is not None and active_mask_path.exists():
        ...
        # Fallback: use only non-NaN returns (i.e. ticker traded that day).
        pv_masked = pv
    ...
    for t in range(window - 1, arr.shape[0]):
        ...
        if cols.size < 5:
            ...
            continue
        sub = win[:, cols]
        # Centered correlation: drop fully-NaN rows pairwise per column.
    ...
    out = pd.Series(
        ...
        name="cs_avg_pairwise_corr_60d_nasdaq100",
    )
    return out
...
def main() -> int:
    ...
    # Rebuild the universal macro using the canonical builder over the
    # extended date range so the SP500 parquet picks up 2014-2025.
    ...
    if not prices_path.exists():
        raise FileNotFoundError(
            ...
            f"build_nasdaq100_prices.py first"
        )
    ...
    nasdaq_macro = macro.copy()
    nasdaq_macro["cs_avg_pairwise_corr_60d_nasdaq100"] = cs_series.reindex(
        nasdaq_macro.index,
    )

    # ---- 5-day release-lag on all macro features. ----
    if args.lag_days > 0:
        nasdaq_macro = nasdaq_macro.shift(args.lag_days)

    # Drop the warm-up window so output starts on the first business day
    # of 2014; the bridge ffills internally.
    nasdaq_macro = nasdaq_macro.loc[
        nasdaq_macro.index >= pd.Timestamp("2014-01-01")
    ].copy()

    DATA_DIR.mkdir(parents=True, exist_ok=True)
    nasdaq_macro.to_parquet(OUT_PATH)
    print(f"[macro] WROTE {OUT_PATH}: shape={nasdaq_macro.shape}; "
          f"{nasdaq_macro.index.min().date()} -> "
          f"{nasdaq_macro.index.max().date()}", flush=True)
    return 0
