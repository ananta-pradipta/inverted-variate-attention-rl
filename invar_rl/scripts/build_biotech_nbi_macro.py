"""Build the biotech NBI daily macro-state vector for Phase 2.
...
A single universe-specific column
``cs_avg_pairwise_corr_60d_biotech_nbi`` is appended: the
cross-sectional average pairwise correlation over a trailing 60-day
window computed from the biotech NBI active-subset log returns.
...
information available by day t-5. Matches the NASDAQ-100, DJIA-30 and
S&P 500 macro pipelines byte-for-byte on the overlap.
...
Inputs:
    ...
    data/biotech_nbi/prices.parquet       (Phase 2 prices)
    data/biotech_nbi/active_mask.parquet  (Phase 2 active mask)
...
def _build_universe_corr(
    ...
    For each business day t, take the active biotech NBI subset (per
    ...
    out = pd.Series(
        ...
        name="cs_avg_pairwise_corr_60d_biotech_nbi",
    )
    return out
...
def main() -> int:
    ...
    if not prices_path.exists():
        raise FileNotFoundError(
            ...
            f"build_biotech_nbi_prices.py first"
        )
    ...
    nbi_macro = macro.copy()
    nbi_macro["cs_avg_pairwise_corr_60d_biotech_nbi"] = cs_series.reindex(
        nbi_macro.index,
    )

    if args.lag_days > 0:
        nbi_macro = nbi_macro.shift(args.lag_days)

    nbi_macro = nbi_macro.loc[
        nbi_macro.index >= pd.Timestamp("2014-01-01")
    ].copy()

    DATA_DIR.mkdir(parents=True, exist_ok=True)
    nbi_macro.to_parquet(OUT_PATH)
    print(f"[macro] WROTE {OUT_PATH}: shape={nbi_macro.shape}; "
          f"{nbi_macro.index.min().date()} -> "
          f"{nbi_macro.index.max().date()}", flush=True)
    return 0
