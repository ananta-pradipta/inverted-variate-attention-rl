"""Build the NASDAQ-100 daily OHLCV price panel for Phase 2.

Reads :mod:`data/nasdaq100/membership.parquet` for the historical
ticker universe (209 unique symbols across 2014-2025) and downloads
daily bars from yfinance with ``auto_adjust=True`` so the close column
is already split- and dividend-adjusted.

Output: ``data/nasdaq100/prices.parquet`` (long-format).

Schema (matches ``data/raw/sp500/prices_sp500.parquet``):
    ticker        (string)
    date          (datetime64[ns])
    open, high, low, close, adj_close   (float64)
    volume        (float64; some yfinance bars return float for new tickers)

The FB -> META rename is resolved via ``aliases.parquet``: we download
the modern ticker (META) only and persist it under both keys (FB rows
before 2022-06-09, META rows from 2022-06-09 onward). yfinance maps
the modern ticker back to its full pre-rename history transparently.

This script is idempotent; it writes to a temp parquet then atomically
renames. Use ``--no-cache`` to force a full re-download.
"""
...
def _fetch(symbol: str, start: str, end: str,
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Expected columns from auto_adjust=True: date, open, high, low,
            # close, volume. No 'adj close' since close is already adjusted.
...
def main() -> int:
    ...
    # Determine which tickers (modern symbols) to actually download:
    # collapse old/new pairs to a single modern symbol so we only hit
    # yfinance once per economic entity, then expand back to as-of-date
    # symbols at persist time.
    new_to_old: dict = {v: k for k, v in aliases.items()}
    ...
    # Optional warm-start: reuse the existing parquet to skip already-downloaded
    # symbols that cover the full requested window.
    ...
    if existing is not None and not args.no_cache:
        # Keep cache rows for symbols we will not re-download.
    ...
    # Expand modern symbols back to as-of-date symbols for any alias pair:
    # rows BEFORE change_date are persisted under the OLD ticker so they
    # align with membership.parquet (which uses as-of-date symbols).
    if ALIASES_PATH.exists():
        al = pd.read_parquet(ALIASES_PATH)
        if not al.empty:
            extra = []
            for _, r in al.iterrows():
                ...
                pre["ticker"] = old_t
                extra.append(pre)
                # Drop the pre-rename rows from the modern-ticker rows so
                # we do not double-count economic days.
                drop_mask = (
                    (merged["ticker"] == new_t)
                    & (merged["date"] < chg)
                )
