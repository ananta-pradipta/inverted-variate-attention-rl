"""Phase 5.5 biotech NBI non-learning baselines (CPU-only).

Direct mirror of :mod:`invar_rl.scripts.nasdaq100_non_learning_baselines`
for the biotech NBI universe. Computes 5 non-learning strategies
analytically from ``data/biotech_nbi/prices.parquet`` +
``data/biotech_nbi/active_mask.parquet`` under the same 5-fold macro-
stratified protocol used everywhere else in InVAR-RL, and writes per-
strategy JSON summaries to
...
Strategies:
  ...
  3. ``momentum_jt_12_2``       : Jegadeesh-Titman 12-1 momentum.
  4. ``reversal_1m``            : 1-month reversal.
...
Usage::
    ...
    python invar_rl/scripts/biotech_nbi_non_learning_baselines.py
"""
...
def _run_one_fold(fold: int) -> dict:
    """Build the biotech NBI bridge for the fold and run all 5 strategies."""
    ...
    print(
        f"[nbi-non-learning] fold={fold} n_test_days={len(day_indices)} "
...
def _pool_sharpe(daily_returns_per_fold: list[np.ndarray]) -> dict:
    """Pool daily log returns across folds and compute annualised Sharpe."""
...
def main() -> int:
    out_root = Path("outputs/biotech_nbi/baselines/non_learning")
    ...
    for strat in _STRATEGIES:
        ...
        payload = {
            ...
            "protocol": (
                ...
                "from data/biotech_nbi/prices.parquet."
            ),
        ...
        print(
            f"[nbi-non-learning] {strat:24s}  pooled Sharpe="
    ...
    print(f"[nbi-non-learning] wrote {pooled_path}")
    return 0
