"""Build the NASDAQ-100 26-col canonical panel for InVAR Layer 1.

Mirrors the lattice_native 26-col schema (see ``src.v2.data.lattice_native_panel.FEATURE_COLS``)
so the canonical InVAR model code can ingest the NASDAQ-100 panel with
no architecture changes (same ``feature_dim = 26``).

Computes the 10 PRICE_VOL columns from ``data/nasdaq100/prices.parquet``
verbatim from ``src.lattice.data.build_panel._compute_price_vol`` math.
Zero-fills the remaining 16 columns (4 distress, 4 intangible, 3 other
fundamental, 3 catalyst, 2 flag) because the NASDAQ-100 universe does
not ship a fundamentals / catalyst feed. The flag columns
``has_fundamentals`` and ``has_stocktwits`` are set to 0 to mark these
slots as universe-missing (the canonical InVAR conditions on these flags
via the duration head, so it sees the same "no fundamentals" signal it
already learned to handle on the S&P 500 tickers without fundamentals).

Output:
    data/nasdaq100/panel_features.parquet
...
def _compute_price_vol_one(sub: pd.DataFrame) -> pd.DataFrame:
    ...
    Math copied verbatim from src.lattice.data.build_panel._compute_price_vol
    so cross-universe parity is byte-for-byte where the underlying OHLCV
    series agrees.

    Args:
        sub: one ticker's OHLCV rows (ticker, date, open, high, low, close,
    ...
    # 5-day forward log return target.
...
def main() -> int:
    ...
    print(
        f"[nasdaq100_panel] prices: {len(prices):,} rows; "
    ...
    # Drop rows with NaN forward return (no horizon target).
    pre_drop = len(panel)
    panel = panel.dropna(subset=["fwd_return_h"]).reset_index(drop=True)
    print(
        f"[nasdaq100_panel] dropped {pre_drop - len(panel):,} rows with "
        f"NaN fwd_return_h; remaining: {len(panel):,}",
        flush=True,
    )

    # NaN-fill PRICE_VOL columns at warm-up edges (the lattice_native
    # builder leaves these as NaN; we zero-fill so the standardiser does
    # not see NaN and corrupt train-fold stats. The trainer's tradable
    # mask filters warm-up days out of the actual loss anyway).
    ...
    # Zero-fill the 16 missing-by-universe columns (4 distress + 4 intang
    # + 3 other-fund + 3 catalyst + 2 flag). Flags = 0 so the duration
    # head sees the "no fundamentals" signal it already learned.
    ...
    print(
        f"[nasdaq100_panel] wrote {OUT_PATH}: "
