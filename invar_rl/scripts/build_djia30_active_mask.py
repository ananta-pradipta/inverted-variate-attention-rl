"""Build the DJIA-30 daily active mask for Phase 2.

A (ticker, date) row is ACTIVE iff ALL of:
  - in_index_flag is True per data/djia30/membership.parquet, AND
  - the ticker has a non-NaN close on that date in
    data/djia30/prices.parquet, AND
  ...
  - the ticker's 20-day rolling average dollar volume on that date is
    >= 1e6 USD (liquidity filter; the DJIA-30 is megacap by
    construction so this gate is almost never binding).

Output: data/djia30/active_mask.parquet
    date     datetime64[ns]
    ticker   string
    active   bool
"""
