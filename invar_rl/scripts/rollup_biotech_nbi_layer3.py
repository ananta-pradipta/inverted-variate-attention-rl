"""Roll up biotech NBI Phase 5 Layer 3 SAC controller results.

Direct mirror of :mod:`invar_rl.scripts.rollup_nasdaq100_layer3` for the
biotech NBI universe. Reads per-cell strategy-return parquets at
``outputs/biotech_nbi/layer3/{ls,lo}/fold*_seed*.parquet`` and aggregates
Sharpe per (protocol, fold) and per-protocol across all 25 cells.

Two pooling formulas are emitted side-by-side:

  * **Per-cell mean** (PRIMARY, matches Phase 5.5 baseline rollup).
...
``reports/biotech_nbi/phase_5_layer3_sharpe.md``.

Usage::

    PYTHONPATH=$PWD python3 -m invar_rl.scripts.rollup_biotech_nbi_layer3
"""
...
def _render_markdown(
    ...
    out_path: Path,
    title: str = "Biotech NBI Phase 5: Layer 3 SAC Sharpe rollup",
    parquet_root: str = "outputs/biotech_nbi/layer3",
) -> None:
...
def main() -> int:
    root = Path("outputs/biotech_nbi/layer3")
    ls_cells = _load_protocol(root / "ls")
    lo_cells = _load_protocol(root / "lo")
    out_path = Path("reports/biotech_nbi/phase_5_layer3_sharpe.md")
