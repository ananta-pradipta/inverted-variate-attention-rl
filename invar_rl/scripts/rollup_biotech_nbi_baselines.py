"""Roll up biotech NBI Phase 5.5 baseline results.

Direct mirror of :mod:`invar_rl.scripts.rollup_nasdaq100_baselines` for
the biotech NBI universe.

Reads:
  - outputs/biotech_nbi/baselines/{master,factorvae,stockmixer,dystage,swa_invar}/foldF_seedS.json
    (Layer-1 ranker baselines; both sharpe_ls and sharpe_lo_native per cell)
  - outputs/biotech_nbi/baselines/invar_l1/foldF_seedS.json
    (InVAR Layer-1 + top-25 L/S wrapper; ours, no QP / no SAC)
  - outputs/biotech_nbi/baselines/{finrl_ppo,finrl_a2c,finrl_ddpg}/foldF_seedS.json
    (FinRL whole-stack RL; long-only top-30 active)
  - outputs/biotech_nbi/baselines/stockformer/foldF_seedS.json
    (StockFormer faithful; long-only top-30 active)
  - outputs/biotech_nbi/baselines/non_learning/{strategy}.json
    (5 deterministic non-learning baselines)

Writes:
  - reports/biotech_nbi/phase_5_5_baselines.md
    (Panel A: ranker baselines; Panel B: whole-stack RL; Panel C:
    non-learning.)

Usage::

    PYTHONPATH=$PWD python3 invar_rl/scripts/rollup_biotech_nbi_baselines.py
"""
...
_OUT_ROOT = Path("outputs/biotech_nbi/baselines")
_REPORT_PATH = Path("reports/biotech_nbi/phase_5_5_baselines.md")
...
def _ranker_section() -> str:
    lines: List[str] = []
    lines.append("## Panel A: Layer-1 ranker baselines (biotech NBI)")
    lines.append("")
    lines.append(
        "Five published ranker baselines plus an InVAR Layer 1 ablation "
        "row, trained on the biotech NBI panel "
        "(`panel_kind=biotech_nbi`, `two_regime_val=True`, "
...
def _rl_section() -> str:
    lines: List[str] = []
    lines.append(
        "## Panel B: Whole-stack RL baselines "
        "(biotech NBI, long-only top-30 active)"
    )
    lines.append("")
    lines.append(
        "FinRL (PPO, A2C, DDPG) and StockFormer (faithful) trained "
        "end-to-end on the top-30 most-active biotech NBI names per "
        "fold, under the InVAR-RL 5-fold macro-stratified protocol. "
        "FinRL: 50k timesteps per cell. StockFormer: 10k SAC + 30 "
        "transformer pretrain epochs per cell."
    )
    lines.append("")
...
def _non_learning_section() -> str:
    lines: List[str] = []
    lines.append("## Panel C: Non-learning baselines (biotech NBI, CPU-only)")
    lines.append("")
    lines.append(
        "Computed analytically from `data/biotech_nbi/prices.parquet` "
        "and `data/biotech_nbi/active_mask.parquet`. Deterministic (no "
        "seed loop); per-fold Sharpe over the test segment plus day-"
        "stream pooled Sharpe across all 5 folds' test segments "
        "concatenated."
    )
    lines.append("")
    lines.append(
...
def _header() -> str:
    return (
        "# Biotech NBI Phase 5.5: baseline replication\n\n"
        "Side-by-side rollup of 5 Layer-1 ranker baselines + an InVAR "
        "Layer 1 ablation + 4 whole-stack RL baselines + 5 non-learning "
        "baselines on the biotech NBI panel, under the InVAR-RL 5-fold "
        "macro-stratified protocol (`panel_kind=biotech_nbi`, "
        "`two_regime_val=True`, `panel_end=2025-12-31`). All baseline "
        "architectures and hyperparameters held byte-for-byte from the "
        "S&P 500 protocol (Policy P1, no retuning).\n\n"
        "All Sharpe numbers use the **per-cell mean** pooling formula "
        "(annualised Sharpe per (fold, seed) cell, then averaged across "
        "cells).\n\n"
    )
