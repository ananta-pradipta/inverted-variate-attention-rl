"""Roll up the A1 sequential regime->sector NDX + NBI sweep.

Reads L2L3 summary JSONs under
``outputs/nasdaq100/layer3_a1_seq_regime_sector/ls/summary/`` and
``outputs/biotech_nbi_enriched/layer3_a1_seq_regime_sector_k25/ls/summary/``
and emits per-universe stats compared to the corresponding canonical
SAC L/S reference (matched to the C3 / A2 cross-universe report
references).

Canonical references (audited per bcbabc2):
    NDX-100               pool +1.194  (F1 +1.97, F2 -0.34, F3 +0.73,
                                         F4 +1.74, F5 +1.87)
...
Stop gates (from the task spec):
    NDX:
        ...
        F2 within +/- 0.10 of canonical -0.34
    NBI:
...
CANONICAL_PER_FOLD: Dict[str, Dict[int, float]] = {
    "nasdaq100": {1: +1.97, 2: -0.34, 3: +0.73, 4: +1.74, 5: +1.87},
...
DEFAULT_LS_DIR: Dict[str, Path] = {
    "nasdaq100": Path(
        "outputs/nasdaq100/layer3_a1_seq_regime_sector/ls"
    ),
    "biotech_nbi_enriched": Path(
        "outputs/biotech_nbi_enriched/"
        "layer3_a1_seq_regime_sector_k25/ls"
    ),
}
...
def roll_universe(
    ...
    for f in FOLDS:
        ...
        per_fold[f] = {
            ...
            "delta": (
                (m - CANONICAL_PER_FOLD[universe][f])
                if m == m else float("nan")
            ),
        }
    ...
    if universe == "nasdaq100":
        f2 = per_fold[2]["mean"]
        canon_f2 = CANONICAL_PER_FOLD[universe][2]
        if f2 == f2:
            f2_delta = f2 - canon_f2
            if abs(f2_delta) <= gates["f2_band"]:
                verdict_flags.append(
                    f"F2 in band ({f2:+.3f} vs {canon_f2:+.3f}, "
                    f"|delta|={abs(f2_delta):.3f} <= {gates['f2_band']:.3f})"
                )
            else:
                verdict_flags.append(
                    f"F2 OUT of band ({f2:+.3f} vs {canon_f2:+.3f}, "
                    f"|delta|={abs(f2_delta):.3f} > {gates['f2_band']:.3f})"
                )
    if universe == "biotech_nbi_enriched":
        ...
        if f45 == f45:
            if f45 >= gates["f45_floor"]:
                verdict_flags.append(
                    f"F4+F5 OK ({f45:+.3f} >= {gates['f45_floor']:+.3f})"
                )
            else:
                verdict_flags.append(
                    f"F4+F5 BELOW FLOOR ({f45:+.3f} < "
                    f"{gates['f45_floor']:+.3f})"
                )
    return {
        "universe": universe,
...
def _fmt_per_fold_table(d: Dict[str, object]) -> str:
    rows = []
    rows.append("| Fold | canonical | A1 seq | delta |")
    rows.append("|---:|---:|---:|---:|")
    for f in FOLDS:
        pf = d["per_fold"][f]
        m = pf["mean"]
...
def main() -> int:
    p = argparse.ArgumentParser(
        description=(
            "Roll up A1 sequential regime->sector NDX + NBI sweep."
        )
    )
    p.add_argument(
        "--ndx-root", type=Path,
    ...
    p.add_argument(
        ...
        default=Path(
            ...
            "a1_seq_regime_sector_ndx_nbi_2026-05-27.json"
        ),
    )
    args = p.parse_args()
