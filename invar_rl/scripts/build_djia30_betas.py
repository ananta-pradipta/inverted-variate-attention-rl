"""Build the DJIA-30 per-(day, ticker) rolling macro-beta panel.

Mirrors ``src.v2.data.rolling_macro_betas.build_rolling_betas`` byte-for-byte
on the DJIA-30 universe. Writes to
``data/processed/djia30_rolling_betas.parquet`` with the SAME
...
``data/djia30/macro.parquet`` which carries 32 of the same 32
universe-shared columns as the S&P 500 macro parquet (byte-identical
on the overlap; see ``reports/djia30/phase_2_report.md``).
"""
...
PRICES_PATH = REPO_ROOT / "data" / "djia30" / "prices.parquet"
MACRO_PATH = REPO_ROOT / "data" / "djia30" / "macro.parquet"
OUT_PATH = REPO_ROOT / "data" / "processed" / "djia30_rolling_betas.parquet"
...
def main() -> int:
    ...
    print(
        f"[djia30_betas] prices: {len(raw):,} rows; "
    ...
    print(
        f"[djia30_betas] panel dates: {len(panel_dates)}; "
    ...
    print(
        f"[djia30_betas] wrote {OUT_PATH}: shape={long.shape}, "
