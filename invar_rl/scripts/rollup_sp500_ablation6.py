"""Roll up SP500 Ablation 6 (macro-conditional concentration) results.

Reads the per-cell strategy-return + K_t parquets at
``outputs/sp500/ablations/ablation6/foldF_seedS.parquet`` and the
per-cell summary JSONs at
``outputs/sp500/ablations/ablation6/summary/foldF_seedS.json``, then
aggregates:
...
1. Per-fold and pooled annualised Sharpe via two pooling formulas
   ...
   :mod:`invar_rl.scripts.rollup_nasdaq100_ablations`.
...
Writes a markdown report to ``reports/sp500/ablation6_concentration.md``.

Usage::

    PYTHONPATH=$PWD python3 -m invar_rl.scripts.rollup_sp500_ablation6
"""
...
# Canonical Option A (fixed K=50 SAC) reference numbers, from
# Sec 5.11 of the universal_RL paper (commit db3db66).
_CANONICAL_FIXED_K_SHARPE = 0.94
...
def _avg_corr_z_for_fold(fold: int) -> Optional[np.ndarray]:
    """Build the SP500 lattice_native bridge for one fold and return avg_corr_z.
...
def _render_markdown(
    ...
    lines: List[str] = []
    lines.append("# SP500 Ablation 6: Macro-Conditional Concentration (2-D SAC)")
    lines.append("")
    lines.append(
        "Tests whether a 2-D SAC action (exposure scalar + concentration "
        "K_t via sigmoid logit) beats the canonical 1-D exposure scalar "
        "with fixed K=50. Mirrors the Option A canonical pipeline "
        "(InVAR Layer 1 + fixed equal-weight L/S wrapper + Layer 2 SAC); "
        "only the SAC action changes."
    )
    lines.append("")
    lines.append("## Action space")
    lines.append("")
    lines.append("- Canonical (Option A): action = (exposure) in [0, 1.5], 1-D, fixed K=50.")
    lines.append(
        "- Ablation 6: action = (exposure, k_logit) in [0, 1.5] x [-5, 5], "
        "2-D. K_t = round(5 + 45 * sigmoid(k_logit)), clipped to [5, 50]."
    )
    lines.append("")
    ...
    fold_groups = _by_fold(cells)
    n_total = sum(len(v) for v in fold_groups.values())
    ...
    # Vs canonical fixed K=50.
    delta = overall_per_cell_mean - _CANONICAL_FIXED_K_SHARPE
    lines.append("## Vs canonical (Option A) fixed K=50 SAC")
    lines.append("")
    lines.append(
        f"| metric                        | canonical (fixed K=50) | Ablation 6 (macro-conditional K) |  delta |"
    )
    lines.append(
        f"|-------------------------------|-----------------------:|---------------------------------:|-------:|"
    )
    ...
    lines.append(
        ...
        "wins; the 1-D exposure scalar is sufficient, fixed K=50 stays "
...
def main() -> int:
    root = Path("outputs/sp500/ablations/ablation6")
    cells = _load_cells(root)
    out_path = Path("reports/sp500/ablation6_concentration.md")
